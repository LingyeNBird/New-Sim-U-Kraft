...
public class MeatShopDailyWorkManager {
    ...
    public static void onServerTick(TickEvent.ServerTickEvent event) {
        ...
        for (ServerLevel level : event.getServer().getAllLevels()) {
            ...
            if (timeOfDay >= 11900 && timeOfDay <= 13100) {
                if (gameTime - lastLogTick >= LOG_INTERVAL_TICKS) {
                    LOGGER.debug("MeatShopDailyWorkManager: 检查傍晚销售任务，当前时间={}", timeOfDay);
                    lastLogTick = gameTime;
                }
            }
            
            // 肉铺销售任务已合并到 CommercialWorkHandler
            // MeatShopOwnerDailyWorkHandler.handleEveningSalesTask(level);
        }
    }
}