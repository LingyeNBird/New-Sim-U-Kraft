...
public class FruitShopDailyWorkManager {
    ...
    public static void onServerTick(TickEvent.ServerTickEvent event) {
        ...
        for (ServerLevel level : event.getServer().getAllLevels()) {
            ...
            if (timeOfDay >= 11800 && timeOfDay <= 12200) {
                if (gameTime - lastLogTick >= LOG_INTERVAL_TICKS) {
                    LOGGER.debug("FruitShopDailyWorkManager: 检查傍晚销售任务，当前时间={}", timeOfDay);
                    lastLogTick = gameTime;
                }
            }
            
            // 水果店销售任务已合并到 CommercialWorkHandler
            // FruitShopOwnerDailyWorkHandler.processEveningSales(level);
        }
    }
}